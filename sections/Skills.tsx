...
const SkillHUD = ({ skill, index }: { skill: Skill; index: number }) => {
    ...
    >
      <div className="relative w-32 h-32 flex items-center justify-center mb-4">
        ...
        <motion.div
          ...
          className="absolute inset-0 m-auto rounded-full border-2 border-t-transparent border-b-transparent border-white/5 w-[85%] h-[85%]"
        />
        <svg 
            className="absolute inset-0 w-full h-full transform -rotate-90 drop-shadow-[0_0_8px_rgba(0,0,0,0.5)]"
            viewBox="0 0 128 128"
        >
          <circle cx="64" cy="64" r={radius} fill="transparent" stroke="rgba(255,255,255,0.05)" strokeWidth="6" />
          <motion.circle
            cx="64"
            cy="64"
            r={radius}
        ...
        <div className="absolute inset-0 flex items-center justify-center z-10">
          <div className={`p-3 rounded-full bg-white/5 backdrop-blur-sm border border-white/10 group-hover:border-${skill.color.split('-')[1]}-500/50 transition-colors duration-300`}>
             <Icon className="w-6 h-6 text-white group-hover:text-primary transition-colors duration-300" />
          </div>
        </div>
        <div className="absolute -bottom-2 bg-surface border border-white/10 px-2 py-0.5 rounded text-[10px] font-mono text-primary shadow-lg z-20">
          {skill.value}%
        </div>
      </div>
      <h3 className="text-white font-medium text-center tracking-wide group-hover:text-primary transition-colors duration-300">
        {skill.label}
      </h3>
    </motion.div>
  );
...
const ToolsCarousel = () => {
  return (
    <div className="mt-24 w-full">
      ...
      <div className="w-full inline-flex flex-nowrap overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-128px),transparent_100%)] bg-white/5 border-y border-white/5 py-8">
